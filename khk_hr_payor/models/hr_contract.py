...
class HrContractBonus(models.Model):
    ...
    convention_id = fields.Many2one('convention.collective.line', 'Categorie')
    nbj_pris = fields.Float("Nombre de jour pris", default="0", store=True)
    ...
    #holidays_tracking = fields.One2many('holidays.tracking', 'contract_id')
...
#                          ]
#                hlds = contract.env['holidays.tracking'].search(domain)
#                if len(hlds) > 1:
...
#            contract.nbj_pris = days
    ...
    def _get_droit(self, provision_conges, provision_fin_contrat):
        for record in self:
            ...
            record.create_allocation(record.company_id.nbj_alloue, 'Leave allowance from payroll')

    @api.depends('cumul_mensuel', 'nbj_pris', 'nbj_aquis')
    def _get_alloc(self):
        for record in self:
            if record.nbj_pris != 0 and record.cumul_mensuel != 0 and record.nbj_aquis != 0:
                _logger.info('IN THE FUNCTION GET ALLOC KHK')
            ...
            if dur.days >= 3653:
                if record.year_extra_day_anciennete:
                    if record.year_extra_day_anciennete != today.year:  # we must add it one time by year
                        ...
                        record.create_allocation(1, 'Leave allowance for seniority')
                else:
                    record.year_extra_day_anciennete = today.year
                    record.write({'nbj_aquis': record.nbj_aquis + 1})
                    record.create_allocation(1, 'Leave allowance for seniority')
    ...
    def create_allocation(self, nb_jour, lib):
        for record in self:
            self.env['hr.leave.allocation'].create({
                'name': lib,
                'holiday_status_id': self.env['hr.leave.type'].search([('paid_legal_leave', '=', True)], order="id desc", limit=1).id,
                'number_of_days': nb_jour,
                'state': 'validate',
                'employee_id': record.employee_id.id
            })

#class HolidaysTracking(models.Model):
#    _name = "holidays.tracking"
#    _description = "Holidays Tracking"
...
#        return super(HolidaysTracking, self).unlink()
