...
class CotisationIpresReport(models.AbstractModel):
    _name = 'report.khk_hr_payor.report_ipres_view'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        contrib_registers = self.env['payor.payslip.lines.cotisation.ipres'].browse(register_ids)
        ...
        for line in self.env['hr.payslip.line'].browse(line_ids):
            ...
            if line.employee_id.id in dico:
                if line.code == 'C1200':  # brut
                    ...
                    line_brut += line.total
                    dico[line.employee_id.id]['Brut'] = line_brut
                elif line.code == 'C2040':  # ipres rc
                    line_ipres_rc += line.total
                    total_ipres_rc += line.total
                    dico[line.employee_id.id]['Ipres_rc'] = line_ipres_rc
                    if line_base_rc == 0.0:
                        total_base_rc += line.amount
                        dico[line.employee_id.id]['Base_rc'] = line.amount
                elif line.code == 'C2030':  # ipres_rg
                    total_ipres_rg += line.total
                    line_ipres_rg += line.total
                    dico[line.employee_id.id]['Ipres_rg'] = line_ipres_rg
                    if line_base_rg == 0.0:
                        total_base_rg += line.amount
                        dico[line.employee_id.id]['Base_rg'] = line.amount
                elif line.code == 'C2041':  # ipres_rc_pat
                    total_ipres_rc_pat += line.total
                    line_ipres_rc_pat += line.total
                    dico[line.employee_id.id]['Ipres_rc_pat'] = line_ipres_rc_pat
                elif line.code == 'C2031':  # ipres_rg_pat
                    total_ipres_rg_pat += line.total
                    line_ipres_rg_pat += line.total
                    dico[line.employee_id.id]['Ipres_rg_pat'] = line_ipres_rg_pat
            else:
                ...
                if line.code == 'C1200':  # brut
                    dico[line.employee_id.id]['Brut'] = line_brut = total_brut = line.total
                elif line.code == 'C2040':  # ipres rc
                    dico[line.employee_id.id]['Ipres_rc'] = line_ipres_rc = total_ipres_rc = line.total
                    dico[line.employee_id.id]['Base_rc'] = total_base_rc = line.amount
                elif line.code == 'C2030':  # ipres_rg
                    dico[line.employee_id.id]['Ipres_rg'] = line_ipres_rg = total_ipres_rg = line.total
                    dico[line.employee_id.id]['Base_rg'] = total_base_rg = line.amount
                elif line.code == 'C2041':  # ipres_rc_pat
                    dico[line.employee_id.id]['Ipres_rc_pat'] =\
                        line_ipres_rc_pat = total_ipres_rc_pat = line.total
                elif line.code == 'C2031':  # ipres_rg_pat
                    dico[line.employee_id.id]['Ipres_rg_pat'] =\
                        line_ipres_rg_pat = total_ipres_rg_pat = line.total
